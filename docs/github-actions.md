...
Windows 任务每 10 分钟以 `--once` 运行一次，读取 `http://127.0.0.1:8001/feed/all.rss`、private-rss 的 `all.xml`、Grok feeds 和 7 个 GitHub 出口容易被 Cloudflare 拦截的 Substack feeds。只有 XML 合法、至少包含一个 item，且 item/entry 的语义指纹发生变化时才提交；`lastBuildDate` 等 feed 级时间戳变化会忽略。we-mp-rss 还要求每个 item 的 `content:encoded` 已经非空，否则保留最后一份好快照并等待下一班，避免文章先出现、正文后补齐时写入短摘要。Substack 镜像抓取失败属于软失败，同样保留最后一份好快照，不把整个发布任务打红。推送成功后触发公开仓库的 `rss-ingest.yml`。
...
- 公网 HTTP(S) 源正常运行。
- `source-map.json` 中显式映射的 18 个飞书 RSS 记录改读私有 XML：we-mp-rss、private-rss、9 个 Grok feed 和 7 个 Substack 镜像。
